...
export class AddGameOverlayComponent implements OnInit {
    ...
    addGame() {
        ...
        this.statusOfImport = this.translate.instant('insertingTheGameIntoTheDatabase');
        this.db.postGame(this.currentGame).then(async (id) => {
            ...
            if (typeof gameIGDB === 'string') {
                this.statusOfImport = this.translate.instant('error') + ' : ' + gameIGDB;
                console.log(gameIGDB);
            } else {
                this.currentGame = gameIGDB;
                this.currentGame.id = id;
                this.statusOfImport = this.translate.instant('found') + ' : ' + this.currentGame.name +
