...
async function getExtraContentPath(): Promise<string> {
	...
	const p = platform();
	if (p === "windows") return configDirPath + "\\Nytuo\\Meteoric\\config\\meteoric_extra_content\\";
	if (p === "macos") return configDirPath + "/fr.Nytuo.Meteoric/meteoric_extra_content/";
	return configDirPath + "/meteoric/meteoric_extra_content/";
}

async function getMusicBasePath(): Promise<string> {
	const p = platform();
	if (p === "windows") return "Nytuo\\Meteoric\\config\\meteoric_extra_content\\";
	if (p === "macos") return "fr.Nytuo.Meteoric/meteoric_extra_content/";
	return "meteoric/meteoric_extra_content/";
}

async function getAllImgAndVideo(id: string, game: IGame, basePath: string): Promise<IGame> {
	const allImgLoc = await invoke<string>("get_all_images_location", { id });
	const parsedImgs = JSON.parse(allImgLoc);
	game.screenshots = parsedImgs.map((loc: string) => convertFileSrc(basePath + loc));
	...
	game.backgroundMusic = (await exists(musicPath, { baseDir: BaseDirectory.Config }))
	...
	await Promise.all(
		games.map(async (game) => {
			...
			try {
				...
				game.jaquette = paths.jaquette ? convertFileSrc(basePath + paths.jaquette) : "";
				game.jaquette_horizontal = paths.jaquette_horizontal ? convertFileSrc(basePath + paths.jaquette_horizontal) : "";
				game.background = paths.background ? convertFileSrc(basePath + paths.background) : "";
				game.logo = paths.logo ? convertFileSrc(basePath + paths.logo) : "";
				game.icon = paths.icon ? convertFileSrc(basePath + paths.icon) : "";
			} catch (e) {
				console.error(`[ERROR] parseGames - Failed to get image paths for game ${id}:`, e);
				game.jaquette = "";
...
export async function refreshGameLinks(game: IGame): Promise<IGame> {
	...
	try {
		...
		game.jaquette = paths.jaquette ? convertFileSrc(basePath + paths.jaquette) + "?" + ts : "";
		game.jaquette_horizontal = paths.jaquette_horizontal ? convertFileSrc(basePath + paths.jaquette_horizontal) + "?" + ts : "";
		game.background = paths.background ? convertFileSrc(basePath + paths.background) + "?" + ts : "";
		game.logo = paths.logo ? convertFileSrc(basePath + paths.logo) + "?" + ts : "";
		game.icon = paths.icon ? convertFileSrc(basePath + paths.icon) + "?" + ts : "";
	} catch (e) {
		console.error(`[ERROR] refreshGameLinks - Failed to get image paths for game ${id}:`, e);
	}
...
export const db = {
	async getGames(): Promise<IGame[]> {
		...
		console.log("[PERF] db.getGames - Raw data size:", (raw.length / 1024).toFixed(2), "KB");
	...
	async createCategory(name: string, icon: string, games: string[], filters: string[], views: string[], background: string) {
		await invoke("create_category", { name, icon, games, filters, views, background });
	},
	...
	async setSettings(settings: ISettings) {
		const arr = Object.entries(settings).map(([name, value]) => ({ name, value }));
