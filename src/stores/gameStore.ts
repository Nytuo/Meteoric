...
interface GameStore {
	...
	searchAPI: (name: string, provider: string, strict: boolean, currentGame?: IGame) => Promise<any[]>;
...
function createNewGame(api: any): IGame {
	return {
		...
		critic_score: api.critic_score ? Math.round(api.critic_score).toString() : "",
		genres: api.genres ?? "",
		styles: api.styles ?? "",
...
export const useGameStore = create<GameStore>((set, get) => ({
	...
	searchGame(query) {
		if (!query) {
			set({ filteredGames: get().games.filter((g) => g.hidden === "false" || !g.hidden) });
			return;
		}
		set({
	...
	filterGames(on = "name", value = "", or_ = "") {
		const { games } = get();
		if (!value) {
			set({ filteredGames: games.filter((g) => g.hidden === "false" || !g.hidden) });
			return;
		}
	...
	async loadRecentPlayed() {
		try {
			...
			played.sort((a, b) => {
				const lastA = a.stats.length > 0 ? Math.max(...a.stats.map((s) => new Date(s.date_of_play).getTime())) : 0;
				const lastB = b.stats.length > 0 ? Math.max(...b.stats.map((s) => new Date(s.date_of_play).getTime())) : 0;
				return lastB - lastA;
			});
	...
	async loadInstalled() {
		try {
			...
			const installed = allGames.filter((g) => {
				// Manual installation: has exec_file and game_dir
				if (g.exec_file && g.game_dir) return true;
				
				// Epic installed: check if app_name is in Epic's installed list
				...
				// GOG installed: check if game_id is in GOG's installed list
			...
			console.log("[PERF] loadInstalled - Total:", allGames.length, "Installed:", installed.length);
			console.log("[PERF] loadInstalled - Epic:", epicInstalledAppNames.size, "GOG:", gogInstalledGameIds.size);
	...
	async loadGameExtras(id: string): Promise<void> {
		try {
			...
			// Only load if not already loaded (screenshots array is empty)
			if (game.screenshots && game.screenshots.length > 0) {
				console.log(`[PERF] Game ${id} extras already loaded, skipping`);
				return;
			}
			
			// Use refreshGameLinks to load screenshots, videos, and music
			const updated = await refreshGameLinks(game);
			
			// Update the game in both games and filteredGames arrays
			const games = get().games.map((g) => (g.id === id ? updated : g));
			const filteredGames = get().filteredGames.map((g) => (g.id === id ? updated : g));
			set({ games, filteredGames });
			
			// Update currentGame if it's the same game
		...
		return [
			{
				name: "Genres",
				values: unique(
					games.flatMap((g) => (g.genres + (g.styles ? "," + g.styles : "")).split(",").filter(Boolean)),
	...
	async launchGame(gameId, launcherId = "") {
		...
		if (launcherId) {
			// Check if this is an Epic game with native launch data (exec_args starts with "epic:")
			if (launcherId === "epic") {
				...
				if (game?.exec_args?.startsWith("epic:")) {
					...
					if (appName) {
						try {
							...
							// Fall through to legacy URL launch
						}
					}
				}
			}

			// Check if this is a GOG game - use native GOG launch instead of URL scheme
			if (launcherId === "gog") {
				try {
			...
			else if (launcherId === "epic") link = "com.epicgames.launcher://apps/" + gameId + "?action=launch&silent=true";
	...
	async searchAPI(name, provider, strict, currentGame?) {
		...
		if (type === "audio") {
			return parsed.map((a: string) => {
				...
				return { name: audio.name ?? "", url: audio.url ?? "", jaquette: audio.jaquette ?? "" };
			});
		}
	...
	async autoDownloadBGMusic(game) {
		...
		try {
			...
			if (results?.length > 0 && results[0].url) {
				await invoke<string>("download_yt_audio", { url: results[0].url, id: game.id });
				const refreshed = await refreshGameLinks(game);
				get().setGame(game.id, refreshed);
			}
		} catch {
			// No audio found, silently fail
		}
	},
