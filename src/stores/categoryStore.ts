...
const STATIC_CATEGORIES: ICategory[] = [
	{ id: "0", name: "All", icon: "home", games: "*", filters: [], views: [], background: "" },
	{ id: "-5", name: "Recent Played", icon: "clock", games: "*", filters: [], views: [], background: "" },
	{ id: "-6", name: "Installed", icon: "download", games: "*", filters: [], views: [], background: "" },
	{ id: "-2", name: "Steam", icon: "bookmark", games: "*", filters: [], views: [], background: "" },
	{ id: "-3", name: "Epic Games", icon: "bookmark", games: "*", filters: [], views: [], background: "" },
	{ id: "-4", name: "GOG", icon: "bookmark", games: "*", filters: [], views: [], background: "" },
];
...
interface CategoryStore {
	...
	createCategory: (name: string, icon: string, games: string[]) => Promise<void>;
...
export const useCategoryStore = create<CategoryStore>((set, get) => ({
	...
	async fetchCategories() {
		try {
			...
			console.log("[PERF] Loaded", allGames.length, "games and", dbCats.length, "custom categories");
			
			let all = [...STATIC_CATEGORIES, ...dbCats];
			all.sort((a, b) => (a.name === "Favorites" ? -1 : b.name === "Favorites" ? 1 : 0));

			console.time("[PERF] fetchCategories - Count Computation");
			// Compute counts in batch using the cached game list
			const countMap = new Map<string, number>();
			countMap.set("All", allGames.filter((g) => g.hidden === "false" || !g.hidden).length);
			countMap.set("Recent Played", allGames.filter((g) => g.stats && g.stats.length > 0).length);
			
			// For Installed count, we need to check Epic/GOG caches too
			// This is a quick count, so we don't fetch - just use what's in memory
			...
			countMap.set("Steam", allGames.filter((g) => g.platforms?.includes("Steam")).length);
			countMap.set("Epic Games", allGames.filter((g) => g.platforms?.includes("Epic Games")).length);
			countMap.set("GOG", allGames.filter((g) => g.platforms?.includes("GOG")).length);

			// Fetch custom category counts in parallel
			const customCats = all.filter((c) => !countMap.has(c.name));
			const customCounts = await Promise.all(
				customCats.map((c) => db.getGamesByCategory(c.name).then((g) => g.length).catch(() => 0))
			);
	...
	async createCategory(name, icon, games) {
		await db.createCategory(name, icon, games, ["All", "Installed", "Not Installed"], ["Grid", "List"], "");
