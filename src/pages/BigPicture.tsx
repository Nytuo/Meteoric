...
function getLastPlayed(game: IGame): string {
  ...
  const sorted = [...game.stats].sort(
    (a, b) =>
      new Date(b.date_of_play).getTime() - new Date(a.date_of_play).getTime()
  );
...
export function BigPicture() {
  ...
  const [detailTab, setDetailTab] = useState<'details' | 'achievements'>(
    'details'
  );
  ...
  const scrollSelectedIntoView = useCallback((idx: number) => {
    setTimeout(() => {
      ...
      if (container && el) {
        ...
        const scrollLeft =
          el.offsetLeft - containerRect.width / 2 + elRect.width / 2;
        container.scrollTo({
  ...
  const navigate_dir = useCallback(
    (dir: 'left' | 'right') => {
  ...
  function handleGamepadButton(button: string) {
    ...
    if (mo) {
      if (button === 'B') setMediaOverlay(null);
      return;
    }
    ...
    if (ss) {
      ...
      } else if (button === 'DPadLeft' || button === 'LB') {
        navigate_dir('left');
      } else if (button === 'DPadRight' || button === 'RB') {
        navigate_dir('right');
      }
      return;
    }
  ...
  const onSearchInput = (q: string) => {
    ...
    setLocalFiltered(
      sourceGames.filter((g) => g.name.toLowerCase().includes(q.toLowerCase()))
    );
    setSelectedIndex(0);
  };
  ...
  const trophies = selectedGame
    ? getTrophyCount(selectedGame)
    : { unlocked: 0, total: 0 };
        ...
        <div className="bp-search-overlay">
          <div className="bp-search-box" onClick={(e) => e.stopPropagation()}>
            ...
            <input
              ...
              placeholder={
                t('bigpicture_search_placeholder') || 'Search games…'
              }
              value={searchQuery}
              onChange={(e) => onSearchInput(e.target.value)}
              autoComplete="off"
            />
            <button className="bp-search-close" onClick={closeSearch}>
              <X size={16} />
            </button>
          </div>
        </div>
        ...
        >
          <div className="bp-exit-dialog" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bp-exit-buttons">
              ...
              >
                {gamepadConnected && <span className="bp-btn-icon">Ⓐ</span>}{' '}
                {t('yes') || 'Yes'}
              </button>
              <button
          ...
          {allGames[selectedIndex] && (
            <div className="sd-home-info">
              {allGames[selectedIndex].logo && (
                <img
                  src={allGames[selectedIndex].logo}
                  className="sd-home-logo"
                  alt={allGames[selectedIndex].name}
                />
              )}
            ...
            {allGames.map((game, i) => (
              ...
              >
                <img
                  src={
                    i === selectedIndex
                      ? game.jaquette_horizontal || game.jaquette
                      : game.jaquette
                  }
              ...
              <div className="sd-action-left">
                ...
                <div className="sd-stat-block">
                  ...
                  <span className="sd-stat-val">
                    {getLastPlayed(selectedGame)}
                  </span>
                </div>
                <div className="sd-stat-block">
                  <span className="sd-stat-label">PLAY TIME</span>
                  <span className="sd-stat-val">
                    {getTotalTimePlayed(selectedGame)}
                  </span>
                </div>
                <div className="sd-stat-block">
                    ...
                    {trophies.total > 0 && (
                      <div className="sd-progress">
                        <div
                          ...
                          style={{
                            width: `${getTrophyPercent(selectedGame)}%`,
                          }}
                        />
                      </div>
                    )}
              ...
              <div className="sd-description-section">
                <div className="sd-description-main">
                  ...
                  <div className="sd-game-meta">
                    <div>
                      Developer:{' '}
                      <span>{selectedGame.developers || 'Unknown'}</span>
                    </div>
                    <div>
                      Publisher:{' '}
                      <span>{selectedGame.editors || 'Unknown'}</span>
                    </div>
                    <div>
                      Release Date:{' '}
                      <span>{selectedGame.release_date || 'Unknown'}</span>
                    </div>
                    <div>
                      Rating: <span>{selectedGame.rating || 'N/A'}</span>
                    </div>
                    <div>
                      Platforms:{' '}
                      <span>{selectedGame.platforms || 'Unknown'}</span>
                    </div>
                    <div>
                      Genres: <span>{selectedGame.genres || 'Unknown'}</span>
                    </div>
                    <div>
                      Styles: <span>{selectedGame.styles || 'Unknown'}</span>
                    </div>
                    <div>
                      Tags: <span>{selectedGame.tags || '—'}</span>
                    </div>
                    <div>
                      Status: <span>{selectedGame.status || 'Unknown'}</span>
                    </div>
                  </div>
                        ...
                        <div className="sd-media-section">
                          ...
                          <div className="sd-media-list sd-video-list">
                            {selectedGame.videos.map((vid, i) => (
                              <div
                                key={i}
                                className="sd-media-item"
                                onClick={() =>
                                  setMediaOverlay({ src: vid, type: 'video' })
                                }
                              >
                                <video
                                  className="sd-video-thumb"
                                  src={vid}
                                  muted
                                  preload="metadata"
                                />
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="bp-achievement-info">
                            <span className="bp-achievement-name">
                              {ach.name || `Achievement ${i + 1}`}
                            </span>
                            {ach.description && (
                              <span className="bp-achievement-desc">
                                {ach.description}
                              </span>
                            )}
                          </div>
