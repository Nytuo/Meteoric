...
import {
	Save, Trash2, X, Eye, EyeOff, Link, Plus, Search,
	FileText, User, Images, Play, Youtube, Monitor, Globe,
} from "lucide-react";
...
export function EditGame() {
	...
	const { getGame, setGame, fetchGames, searchAPI, loadGameExtras } = useGameStore();
	const { changeSidebarOpen, stopAllAudio, changeBlockUI, downloadYTAudio } = useAppStore();
	...
	const generalKeys = ["name", "sort_name", "rating", "platforms", "tags", "description", "critic_score", "genres", "styles", "release_date", "developers", "editors"];
	...
	const linkGame = async () => {
		...
		await db.postGame(updated);
		setGame(id, updated);
		setFormData((prev) => ({ ...prev, exec_file: updated.exec_file, game_dir: updated.game_dir, exec_args: "" }));
	...
	const toggleHidden = async () => {
		if (!id || !game) return;
		const updated = { ...game, hidden: game.hidden === "true" ? "false" : "true" };
		await db.postGame(updated);
		setGame(id, updated);
	...
	const onFileSelected = async (e: React.ChangeEvent<HTMLInputElement>, type: string) => {
		const file = e.target.files?.[0];
		if (!file || !id || !game) { toast.error(t("no-file-selected")); return; }
	...
	const searchGameInAPI = async () => {
		...
		if (!result) { toast.error("No results"); return; }
	...
	const selectItem = async (overrideItem?: any) => {
		...
		if (!item || !id) { toast.error(t("no-file-selected")); return; }
		changeBlockUI(true);
		item.id = id;
	...
	const addSession = () => {
		...
		stats.push({ date_of_play: new Date(), time_played: "0", id: (stats.length + 1).toString(), game_id: id });
	...
	const menuItems = [
		{
			...
			items: [
				{ label: t("general"), icon: <FileText className="h-4 w-4" />, key: "general" },
				{ label: t("personal"), icon: <User className="h-4 w-4" />, key: "personal" },
				{ label: t("media"), icon: <Images className="h-4 w-4" />, key: "media" },
				{ label: t("execution"), icon: <Play className="h-4 w-4" />, key: "exec" },
			],
		},
		{
			label: t("metadata-providers"),
			items: [
				{ label: t("youtube-background-music-provider"), icon: <Youtube className="h-4 w-4" />, key: "ytdl" },
				{ label: t("steam-grid-db-provider"), icon: <Monitor className="h-4 w-4" />, key: "steam_grid" },
				{ label: t("igdb-provider"), icon: <Globe className="h-4 w-4" />, key: "igdb" },
			],
		},
	];

	const isProviderSearch = ["igdb", "ytdl", "steam_grid"].includes(selectedProvider);
		...
		<div className="flex h-full">
			...
			<div className="w-60 shrink-0 border-r border-border bg-card/50 p-4">
				<p className="mb-3 text-xs text-muted-foreground">
					{t("editing-currentgame-name")} <span className="font-medium text-foreground">{game?.name}</span>
				</p>
									...
									{section.items.map((item) => (
										<button
											...
											onClick={() => {
												...
												setSearchedGames([]);
												setShowSearch(isProviderSearch || ["ytdl", "steam_grid", "igdb"].includes(item.key));
											}}
				...
				<div className="flex gap-2">
					<Button variant="outline" size="sm" onClick={() => navigate(`/game/${id}`)}>
				...
				>
					{formData.hidden === "true" ? <Eye className="mr-1 h-3 w-3" /> : <EyeOff className="mr-1 h-3 w-3" />}
			...
			<ScrollArea className="flex-1">
				<div className="p-6">
						...
						<div className="mb-6">
							<div className="flex items-center gap-3 mb-4">
								...
								<div className="mt-6 flex items-center gap-2">
									<Checkbox id="strict" checked={strict} onCheckedChange={(v) => setStrict(!!v)} />
									<Label htmlFor="strict">{t("strict")}</Label>
								</div>
								{selectedProvider !== "ytdl" && (
									<Button onClick={() => selectItem()} className="mt-6" variant="secondary">
										{t("select")}
									</Button>
								)}
							</div>

					{selectedProvider === "igdb" && <IGDBResults results={searchedGames} onSelect={setSelectedItem} selectedItem={selectedItem} />}
					{selectedProvider === "steam_grid" && <SteamGridResults results={searchedGames} onSelect={(item) => selectItem(item)} />}
					{selectedProvider === "ytdl" && <YTDLResults results={searchedGames} onSelect={(item) => selectItem(item)} />}
						</div>
					)}
					...
					{selectedProvider === "general" && (
						<div>
							<h2 className="mb-4 text-xl font-semibold">{t("general-info")}</h2>
								...
								{generalKeys.map((key) => (
									<div key={key}>
										...
										<Input
											...
											className="mt-1"
										/>
										{key === "tags" && <p className="mt-1 text-xs text-muted-foreground">{t("tags-are-separated-by-commas")}</p>}
									</div>
								))}
							</div>
					...
					{selectedProvider === "personal" && (
						<div>
							<h2 className="mb-4 text-xl font-semibold">{t("personal-stats")}</h2>
							<div className="space-y-4">
								<div>
									...
									<Input
										...
										onChange={(e) => updateField("trophies_unlocked", e.target.value)}
								...
								<div>
									...
									{(formData.stats || []).map((session: any, i: number) => (
										<div key={session.id || i} className="mb-3 flex items-center gap-3">
											<div>
												...
												<Input
													...
													onChange={(e) => {
														const stats = [...(formData.stats || [])];
														stats[i] = { ...stats[i], time_played: e.target.value };
											...
											<div>
												...
												<Input
													...
													onChange={(e) => {
														const stats = [...(formData.stats || [])];
														stats[i] = { ...stats[i], date_of_play: e.target.value };
														updateField("stats", stats);
													}}
													className="mt-1 w-56"
												/>
											</div>
											<Button variant="destructive" size="icon" className="mt-5 rounded-full" onClick={() => deleteSession(session.id)}>
												<X className="h-4 w-4" />
											</Button>
										</div>
									))}
					...
					{selectedProvider === "media" && (
						<div>
							...
							<div className="space-y-8">
								{/* Background */}
								<div>
									<h3 className="mb-2 text-sm font-medium">{t("background")}</h3>
									{formData.background && <img src={formData.background} alt="" className="mb-2 h-32 rounded-lg object-cover" />}
									<input type="file" onChange={(e) => onFileSelected(e, "background")} className="text-xs" />
								</div>
								{/* Jaquette */}
								<div>
									<h3 className="mb-2 text-sm font-medium">{t("jaquette")}</h3>
									{formData.jaquette && <img src={formData.jaquette} alt="" className="mb-2 h-40 rounded-lg object-cover" />}
									<input type="file" onChange={(e) => onFileSelected(e, "jaquette")} className="text-xs" />
								</div>
								{/* Logo */}
								<div>
									<h3 className="mb-2 text-sm font-medium">{t("logo")}</h3>
									{formData.logo && <img src={formData.logo} alt="" className="mb-2 h-32 rounded-lg object-contain" />}
									<input type="file" onChange={(e) => onFileSelected(e, "logo")} className="text-xs" />
								</div>
								{/* Icon */}
								<div>
									<h3 className="mb-2 text-sm font-medium">{t("icon")}</h3>
									{formData.icon && <img src={formData.icon} alt="" className="mb-2 h-20 rounded-lg object-contain" />}
									<input type="file" onChange={(e) => onFileSelected(e, "icon")} className="text-xs" />
								</div>
								{/* Screenshots */}
								<div>
									<h3 className="mb-2 text-sm font-medium">{t("screenshots")}</h3>
									<input type="file" onChange={(e) => onFileSelected(e, "screenshot")} className="mb-2 text-xs" />
									<div className="flex flex-wrap gap-3">
										{(formData.screenshots || []).map((ss: string, i: number) => (
											<div key={i} className="group relative">
												<img src={ss} alt="" className="h-32 rounded-lg object-cover" />
												<Button
													variant="destructive"
													size="icon"
								...
								{/* Background music */}
								<div>
									<h3 className="mb-2 text-sm font-medium">{t("background-music")}</h3>
									<input type="file" onChange={(e) => onFileSelected(e, "audio")} className="mb-2 text-xs" />
									<div className="flex items-center gap-3">
										<Input placeholder={t("youtube-video-url")} value={ytUrl} onChange={(e) => setYtUrl(e.target.value)} className="max-w-sm" />
										<Button size="icon" onClick={searchYTBGMusic}><Save className="h-4 w-4" /></Button>
										<Button size="icon" variant="destructive" onClick={deleteBackgroundMusic}><Trash2 className="h-4 w-4" /></Button>
									</div>
									{formData.backgroundMusic && <audio controls src={formData.backgroundMusic} className="mt-2" />}
								</div>
								{/* Videos */}
								<div>
									<h3 className="mb-2 text-sm font-medium">{t("video")}</h3>
									<input type="file" onChange={(e) => onFileSelected(e, "video")} className="mb-2 text-xs" />
									{(formData.videos || []).map((vid: string, i: number) => (
										<div key={i} className="group relative mb-3">
											<video controls src={vid} className="max-w-md rounded-lg" />
											<Button
												variant="destructive"
												size="icon"
												className="absolute -right-2 -top-2 h-6 w-6 rounded-full opacity-0 group-hover:opacity-100"
					...
					{selectedProvider === "exec" && (
						<div>
							...
							<Button onClick={linkGame} className="mb-4 bg-purple-600 hover:bg-purple-700">
