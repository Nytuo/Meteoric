...
function determinePlatformSvg(
  platforms: string
): { type: 'svg'; svg: string } | { type: 'img'; src: string } | null {
...
export function GameDetails() {
  ...
  const { syncAchievements: steamSyncAchievements } = useSteamStore();
  ...
  const epicAppName =
    isEpic && game?.exec_args ? (game.exec_args.split(':')[1] ?? '') : '';
  const epicNamespace =
    isEpic && game?.exec_args ? (game.exec_args.split(':')[2] ?? '') : '';
  const epicEntry = isEpic
    ? (downloadableGames.find((g) => g.app_name === epicAppName) ?? null)
    : null;

  const isGog = game?.importer_id === 'gog';

  const gogGameId =
    isGog && game?.exec_args ? (game.exec_args.split(':')[1] ?? '') : '';
  ...
  const totalTimePlayed = toParsedTime(
    Math.floor(
      (game.stats?.reduce(
        (acc, s) => acc + parseInt(s.time_played || '0'),
        0
      ) || 0) / 60000
    )
  );
  const lastTimePlayed = game.stats?.length
    ? game.stats
        .reduce((a, b) =>
          new Date(a.date_of_play) > new Date(b.date_of_play) ? a : b
        )
        .date_of_play?.toLocaleString?.() || 'N/A'
    : 'N/A';
  ...
  const hltbItems = [
    {
      label: 'Main Story',
      value: hltbData.mainStory,
      Icon: BookOpen,
      hide: hltbData.mainStory === 'N/A',
    },
    {
      label: 'Main + Extras',
      value: hltbData.mainExtra,
      Icon: Swords,
      hide: hltbData.mainExtra === 'N/A',
    },
    {
      label: 'Completionist',
      value: hltbData.completionist,
      Icon: Trophy,
      hide: hltbData.completionist === 'N/A',
    },
    {
      label: 'All Styles',
      value: hltbData.allStyles,
      Icon: Clock,
      hide: hltbData.allStyles === 'N/A',
    },
    {
      label: 'Co-op',
      value: hltbData.coop,
      Icon: Users,
      hide: hltbData.coop === 'N/A',
    },
    {
      label: 'Versus',
      value: hltbData.versus,
      Icon: Gamepad2,
      hide: hltbData.versus === 'N/A',
    },
  ].filter((it) => !it.hide);
    ...
    <div className="relative h-full overflow-hidden">
        ...
        <div className="min-h-screen px-8 pb-16 pt-10">
                ...
                <div className="flex items-center gap-0.5">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        handleRatingChange(star === rating ? 0 : star)
                      }
          ...
          <div className="mb-8 grid grid-cols-4 gap-4">
            <InfoCard
              icon="clock"
              color="green"
              title={t('total-time-played') || 'Total Time Played'}
              description={totalTimePlayed || '0'}
            />
            <InfoCard
              icon="trophy"
              color="orange"
              title={t('trophies-unlocked') || 'Trophies Unlocked'}
              description={game.trophies_unlocked || '0'}
            />
            <InfoCard
              icon="calendar"
              color="red"
              title={t('last-time-played') || 'Last Time Played'}
              description={lastTimePlayed}
            />
            <InfoCard
              icon="tag"
              color="blue"
              title={t('tags') || 'Tags'}
              description={gameTags}
            />
          </div>
            ...
            <div className="mb-8">
              ...
              {cloudLoading ? (
                <div className="flex items-center gap-2 rounded-xl border bg-muted/20 p-4 text-sm text-muted-foreground">
                  <Loader2 className="h-4 w-4 animate-spin" /> Checking cloud
                  save status…
                </div>
              ) : cloudInfo ? (
                <div className="rounded-xl border bg-muted/20 p-4">
                  <p className="text-sm font-medium">
                    {cloudInfo.status?.replace(/([A-Z])/g, ' $1').trim() ??
                      'Unknown'}
                  </p>
              ...
              <h3 className="mb-4 flex items-center gap-2 text-sm font-semibold uppercase tracking-wider text-muted-foreground">
                ...
                {achievements.length > 0 && (
                  <span className="ml-2 text-xs font-normal normal-case">
                    {
                      achievements.filter(
                        (a) => a.unlocked === '1' || a.unlocked === 'true'
                      ).length
                    }{' '}
                    / {achievements.length}
                  </span>
              ...
              {achievements.length > 0 ? (
                <div className="grid grid-cols-2 gap-2">
                  {achievements.map((ach, i) => {
                    const isUnlocked =
                      ach.unlocked === '1' || ach.unlocked === 'true';
                    return (
                      <div
                        key={i}
                        ...
                        {ach.image_url_unlocked ? (
                          <img
                            src={
                              isUnlocked
                                ? ach.image_url_unlocked
                                : ach.image_url_locked || ach.image_url_unlocked
                            }
                            alt=""
                          ...
                          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded bg-muted">
                            <Trophy
                              className={`h-5 w-5 ${isUnlocked ? 'text-yellow-400' : 'text-muted-foreground'}`}
                            />
                          </div>
                        )}
                ...
                <p className="text-sm text-muted-foreground">
                  No achievements synced yet. Press Sync to load them.
                </p>
              )}
            </div>
          )}
                ...
                {videos.length > 1 && (
                  <div className="absolute inset-y-0 flex w-full items-center justify-between px-2">
                    ...
                    <Button
                      ...
                      className="h-8 w-8 rounded-full opacity-70 hover:opacity-100"
                      onClick={() =>
                        setVideoIndex((i) => Math.min(videos.length - 1, i + 1))
                      }
      ...
      {showLaunchAnim && game && (
        <div className="pointer-events-none fixed inset-0 z-50 flex items-center justify-center">
          <div
            className="absolute inset-0 animate-pulse bg-black/40 backdrop-blur-sm"
            style={{ animation: 'fadeOut 2.5s forwards' }}
          />
          <div
            className="relative flex flex-col items-center gap-4 rounded-2xl border border-border/50 bg-black/70 px-12 py-8 shadow-2xl backdrop-blur-md"
          ...
          >
            {game.logo ? (
              <img
                src={game.logo}
                alt={game.name}
                className="max-h-28 max-w-52 object-contain drop-shadow-2xl"
              />
            ) : (
