...
export function CsvImporter() {
	...
	const handleImport = async () => {
		...
		try {
			...
			await fetchGames();
			toast.success(t("dataImportedSuccessfully") || "Data imported successfully");
		} catch (e: any) {
	...
	return (
		<div>
			...
			<p className="mb-4 text-sm text-muted-foreground">
				{t("csv-importer") || "Import games from a CSV file and map columns to database fields."}
			</p>
			...
			{csvHeaders.length > 0 && (
				<>
					...
					<div className="mb-4 max-h-60 space-y-2 overflow-y-auto pr-1">
						{csvHeaders.map((header) => (
							<div key={header} className="flex items-center gap-3">
								<Label className="w-40 shrink-0 truncate text-sm">{header}</Label>
								<Select
									value={mapping[header] || "__none__"}
									onValueChange={(val) =>
										setMapping((prev) => ({ ...prev, [header]: val === "__none__" ? "" : val }))
									}
								>
									...
									<SelectContent>
										<SelectItem value="__none__">— {t("skip") || "skip"} —</SelectItem>
										{dbColumns.map((col) => (
					...
					<Button onClick={handleImport} disabled={loading} className="mb-6">
						{loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Import className="mr-2 h-4 w-4" />}
						{t("importToDatabase") || "Import to Database"}
					</Button>

					<h3 className="mb-2 font-medium">{t("preview") || "Preview"} ({csvRows.length} rows)</h3>
