              ...
              {authStep === 0 && (
                <div className="space-y-4">
                  <p className="text-sm text-muted-foreground">
                    {t('gog-importer.redirectwarning') ||
                      'You will be redirected to GOG to authorize access.'}
                  </p>
                  <p className="text-sm text-muted-foreground">
                    {t('gog-importer.alreadyConnected') ||
                      'If already connected, proceed to the next step.'}
                  </p>
              ...
              {authStep === 1 && (
                <div className="space-y-4">
                  <div className="max-w-sm">
                    <Label htmlFor="gog-auth">
                      {t('authcode') || 'Authorization Code'}
                    </Label>
                    <Input
                      id="gog-auth"
                      value={authCode}
        ...
        <div className="flex flex-1 flex-col space-y-3 overflow-hidden">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">
              {downloadableGames.length} games
            </span>
          </div>
          ...
          {!loggedIn && (
            <p className="text-sm text-amber-600 dark:text-amber-400">
              ...
              {t('pleaseLoginFirst') ||
                'Please login first to see your GOG library.'}
            </p>
          )}
            ...
            <div className="space-y-1 pr-3">
              {downloadableGames.map((game) => {
                const localGame = games.find((g) => {
                  ...
                  return (
                    g.importer_id === 'gog' && gogData?.[1] === game.game_id
                  );
                });
                const hasLocalExecutable = !!(
                  localGame?.exec_file && localGame?.game_dir
                );
              ...
              {downloadableGames.length === 0 && !loadingGames && (
                <p className="py-4 text-center text-sm text-muted-foreground">
                  {t('noGamesFound') ||
                    'No games found. Sync your library first.'}
                </p>
              )}
            </div>
          </ScrollArea>
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      <span className="ml-2">
                        {(game.install_size / 1024 / 1024 / 1024).toFixed(1)} GB
                      </span>
                    </p>
                  </div>
                  <div className="flex gap-1">
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => uninstallGame(game.game_id)}
                    >
