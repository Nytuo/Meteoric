...
export function Topbar() {
  ...
  const epicInstalled = isEpic
    ? (installedGames.find((g) => g.app_name === epicAppName) ?? null)
    : null;
  ...
  const gogInstalled = isGog
    ? (gogInstalledGames.find((g) => g.game_id === gogGameId) ?? null)
    : null;
  const gogEntry = isGog
    ? (gogDownloadableGames.find((g) => g.game_id === gogGameId) ?? null)
    : null;
  ...
  const hasExecutable = (g: any) =>
    !!(
      (g?.exec_file && g?.game_dir) ||
      (g?.game_importer_id && g?.importer_id)
    );
  ...
  const isCategoryChecked = (catId: string) =>
    gameCategories.some((c) => c.id === catId);
  ...
  const handleCloudCheck = async () => {
    ...
    try {
      if (isGog) {
        ...
        const info = await checkCloudSaves(
          epicAppName || game.game_importer_id
        );
        setCloudInfo(info);
      }
    } catch (e: any) {
  ...
  return (
    <>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  size="icon"
                  className="h-8 w-8"
                  onClick={() => setGameOverlayOpen(true)}
                >
      ...
      {gameOverlayOpen && (
        <AddGameOverlay
          open={gameOverlayOpen}
          onOpenChange={setGameOverlayOpen}
        />
      )}
              ...
              <>
                ...
                {epicEntry?.install_size && (
                  <p className="text-xs text-muted-foreground">
                    Size:{' '}
                    {(epicEntry.install_size / 1024 / 1024 / 1024).toFixed(1)}{' '}
                    GB
                  </p>
                )}
                <div className="flex justify-end gap-2">
                  ...
                  <Button
                    onClick={isGog ? handleInstallGog : handleInstallEpic}
                  >
          ...
          <div className="space-y-3 pt-2">
            ...
            <div className="flex gap-2">
              <Button
                ...
                onClick={() => {
                  if (isGog)
                    gogUploadSaves(gogGameId, cloudInfo?.local_save_path ?? '');
