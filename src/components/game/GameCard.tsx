...
export function GameCard({ game }: GameCardProps) {
	...
	const parsedTags = game.tags ? game.tags.split(",").map((t) => t.trim()).filter(Boolean) : [];
		...
		>
			...
			<div className="mt-2 px-1">
				...
				{displayInfo === "platforms" && (
					<p className="truncate text-xs italic text-muted-foreground">{game.platforms}</p>
				)}
				{displayInfo === "rating" && (
					<div className="flex items-center gap-0.5">
						{[1, 2, 3, 4, 5].map((star) => (
							<svg
								key={star}
								className={cn("h-3 w-3", parseInt(game.rating || "0") >= star ? "text-yellow-400 fill-yellow-400" : "text-muted-foreground")}
								viewBox="0 0 24 24"
							>
