...
pub async fn download_game(
    ...
    for (filename, file_manifest) in &files {
        ...
        for chunk_part in &file_manifest.file_chunk_parts {
            ...
            let chunk_offset = chunk_part.offset as u64;
            ...
            if !all_chunks.contains_key(&guid) {
                let url = if let Some(link) = &chunk_part.link {
                    ...
                    let hash = download_manifest
                        .chunk_hash_list
                        .get(&guid)
                        .copied()
                        .unwrap_or(0);
                    let group = download_manifest
                        .data_group_list
                        .get(&guid)
                        .copied()
                        .unwrap_or(0);
    ...
    for batch in chunk_list.chunks(batch_size) {
        ...
        for task in download_tasks {
            let result: Result<(String, Vec<u8>, u64), String> =
                task.await.map_err(|e| format!("Task join error: {}", e))?;
...
fn decompress_chunk(data: &[u8]) -> Vec<u8> {
    ...
    if magic != 0xB1FE3AA2 {
        ...
        if let Ok(decompressed) = try_zlib_decompress(data) {
            eprintln!(
                "[DEBUG] Raw zlib decompressed to {} bytes",
                decompressed.len()
            );
            return decompressed;
        }
