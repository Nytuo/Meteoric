...
mod tests {
    ...
    fn test_query_data() {
        ...
        conn.execute(
            "INSERT INTO games (name, rating) VALUES ('Test Game', '5')",
            [],
        )
        .unwrap();
    ...
    fn test_add_game_to_category_db() {
        let conn = setup_test_db();
        conn.execute(
            "INSERT INTO category (id, name, games) VALUES (1, 'Action', '')",
            [],
        )
        .unwrap();
    ...
    fn test_remove_game_from_category_db() {
        let conn = setup_test_db();
        conn.execute(
            "INSERT INTO category (id, name, games) VALUES (1, 'Action', '123,456')",
            [],
        )
        .unwrap();
    ...
    fn test_modify_table_add_missing_columns() {
        ...
        .unwrap();
        modify_table_add_missing_columns(
            &conn,
            "test_table",
            vec![
    ...
    fn test_required_columns_exist() {
        ...
        let category_columns_exist =
            check_columns_exist(&conn, "category", &required_columns_category);
    ...
    pub(crate) fn check_columns_exist(
        conn: &Connection,
        table_name: &str,
